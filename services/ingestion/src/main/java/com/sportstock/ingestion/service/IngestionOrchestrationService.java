...
public class IngestionOrchestrationService {
  ...
  public void runFoundationSync(Integer seasonYear, Integer seasonType, Integer week) {
    ...
    try {
      log.info(
          "Starting foundation sync for season {} type {} week {}", seasonYear, seasonType, week);
    ...
    try {
      ...
      for (var event : events) {
        try {
          ...
          log.error(
              "Failed to ingest event summary for event {}: {}", event.espnId(), e.getMessage());
        }
      }
      log.info(
          "Weekly sync complete: {} events succeeded, {} failed{}",
          success,
          failed,
