...
class EventIngestionControllerTest {
  ...
  void listEventsReturns400WhenWeekNumberProvidedWithoutSeasonType() throws Exception {
    mockMvc
        .perform(
            get("/api/v1/ingestion/events").param("seasonYear", "2025").param("weekNumber", "1"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code").value("BAD_REQUEST"))
        .andExpect(
            jsonPath("$.message").value("seasonType is required when weekNumber is provided"));
  }
  ...
  void listEventsReturns200WhenOnlySeasonYearProvided() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/ingestion/events").param("seasonYear", "2025"))
        .andExpect(status().isOk());
