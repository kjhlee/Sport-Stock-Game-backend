...
public class TransactionServiceClient {
    ...
    public WalletResponse createWallet(Long userId, Long leagueId) {
        try {
            return transactionRestClient.post()
                    .uri("/api/v1/wallets")
            ...
            log.error("Failed to create wallet for user {} in league {}: {}", 
                      userId, leagueId, e.getMessage());
            throw new RuntimeException("Transaction service unavailable", e);
        }
    }

    public StipendResultResponse issueInitialStipends(Long leagueId, 
                                                      BigDecimal amount, 
                                                      List<Long> userIds) {
        try {
            return transactionRestClient.post()
            ...
            log.error("Failed to issue initial stipends for league {}: {}", 
                      leagueId, e.getMessage());
            throw new RuntimeException("Transaction service unavailable", e);
        }
    }
    ...
    public WalletResponse getWallet(Long userId, Long leagueId) {
        try {
            return transactionRestClient.get()
            ...
            log.error("Failed to fetch wallet for user {} in league {}: {}", 
                      userId, leagueId, e.getMessage());
            throw new RuntimeException("Transaction service unavailable", e);
        }
    }
