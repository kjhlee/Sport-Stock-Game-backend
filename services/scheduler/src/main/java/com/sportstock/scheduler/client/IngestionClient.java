...
public class IngestionClient {
  ...
  public CurrentWeekResponse getCurrentWeek() {
    return restClient.get().uri("/seasons/current-week").retrieve().body(CurrentWeekResponse.class);
  }

  public CurrentWeekResponse getPriorWeek() {
    try {
      return restClient.get().uri("/seasons/prior-week").retrieve().body(CurrentWeekResponse.class);
    } catch (Exception e) {
  ...
  public void syncAllTeamDetails(int seasonYear) {
    restClient
        ...
        .uri(
            uriBuilder ->
                uriBuilder.path("/sync/teams/details").queryParam("seasonYear", seasonYear).build())
        .retrieve()
        .toBodilessEntity();
