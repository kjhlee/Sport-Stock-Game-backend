...
public class StockMarketClient {
  ...
  public PriceUpdateResponse updateProjectedPrices(int seasonYear, int seasonType, int weekNumber) {
    return restClient
        .post()
        .uri(
            uriBuilder ->
  ...
  public int lockEvent(String eventEspnId) {
    Map<String, Integer> body =
        restClient
            ...
            .uri(
                uriBuilder ->
                    uriBuilder.path("/lock-event").queryParam("eventEspnId", eventEspnId).build())
            .retrieve()
  ...
  public void syncInjuries(int seasonYear) {
    restClient
        ...
        .uri(
            uriBuilder ->
                uriBuilder.path("/sync-injuries").queryParam("seasonYear", seasonYear).build())
        .retrieve()
        .toBodilessEntity();
  ...
  public SyncAthletesResponse syncTeamDefenseStocks() {
    return restClient.post().uri("/sync-team-defense").retrieve().body(SyncAthletesResponse.class);
  }
}
