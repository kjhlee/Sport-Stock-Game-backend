...
public class GameDayPollingJob {
    ...
    try {
      ...
      Map<String, EventState> stateMap =
          eventStateRepository
              .findByWeekNumberAndSeasonYearAndSeasonType(weekNumber, seasonYear, seasonType)
              .stream()
              .collect(Collectors.toMap(EventState::getEventEspnId, Function.identity()));
      ...
      log.warn(
          "Skipping game-day polling cycle because dependencies are unavailable: {}",
          e.getMessage());
    }
  }
  ...
  private void processEvent(
    ...
    if (STATUS_POST.equalsIgnoreCase(statusState) || Boolean.TRUE.equals(event.statusCompleted())) {
      handleCompleted(espnId, state, weekNumber, seasonYear, seasonType);
    }
  }

  private void handlePreKickoffLock(
      String espnId, EventState state, int weekNumber, int seasonYear, int seasonType) {
    if (state != null) {
      return;
    }
  ...
  private void handleInProgress(
    ...
    if (STATE_LOCKED.equals(state.getStatus()) || STATE_IN_PROGRESS.equals(state.getStatus())) {
      try {
