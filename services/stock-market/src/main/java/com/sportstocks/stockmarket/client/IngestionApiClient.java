package com.sportstocks.stockmarket.client;
...
public class IngestionApiClient {
    ...
    private static final ParameterizedTypeReference<List<IngestionEventDto>> EVENT_LIST =
            new ParameterizedTypeReference<>() {};
    private static final ParameterizedTypeReference<List<IngestionPlayerGameStatsDto>> PLAYER_STATS_LIST =
            new ParameterizedTypeReference<>() {};
    ...
    public List<IngestionTeamDto> getTeams() {
        List<IngestionTeamDto> body = restClient.get()
                .uri("/teams")
                .retrieve()
                .body(TEAM_LIST);
    ...
    public List<IngestionEventDto> getEvents(int seasonYear, int seasonType, int weekNumber) {
        List<IngestionEventDto> body = restClient.get()
     ...
     *
     * Returns one entry per stat category per player.
     * Caller must merge by athleteEspnId.
     */
    public List<IngestionPlayerGameStatsDto> getPlayerStats(String eventEspnId) {
        List<IngestionPlayerGameStatsDto> body = restClient.get()
