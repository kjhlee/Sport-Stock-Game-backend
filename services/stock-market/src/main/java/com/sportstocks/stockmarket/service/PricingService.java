...
public class PricingService {
    ...
    public PriceUpdateResult updatePricesForWeek(int seasonYear, int seasonType, int weekNumber) {
        ...
        List<IngestionEventDto> events = ingestionApiClient.getEvents(seasonYear, seasonType, weekNumber);
        ...
        Map<String, List<IngestionPlayerGameStatsDto>> statsByAthlete = events.stream()
        ...
        for (Map.Entry<String, List<IngestionPlayerGameStatsDto>> entry : statsByAthlete.entrySet()) {
            ...
            log.info("Updated price for athlete {} ({}) to {}", athleteEspnId, stock.getFullName(), newPrice);
            updated++;
        }
    ...
    private BigDecimal computePerformanceScore(String position, List<IngestionPlayerGameStatsDto> stats) {
        if (position == null) {
    ...
    private BigDecimal scoreQb(List<IngestionPlayerGameStatsDto> stats) {
        ...
        for (IngestionPlayerGameStatsDto stat : stats) {
            ...
            switch (stat.getStatCategory().toLowerCase()) {
                case "passing" -> {
                    score = score.add(getStat(s, "passingYards").divide(BigDecimal.valueOf(25), 4, RoundingMode.HALF_UP));
                ...
                case "rushing" -> {
                    score = score.add(getStat(s, "rushingYards").divide(BigDecimal.valueOf(10), 4, RoundingMode.HALF_UP));
        ...
        for (IngestionPlayerGameStatsDto stat : stats) {
            ...
            switch (stat.getStatCategory().toLowerCase()) {
                ...
                case "receiving" -> {
                    ...
                    score = score.add(getStat(s, "receivingYards").divide(BigDecimal.valueOf(10), 4, RoundingMode.HALF_UP));
    ...
    private BigDecimal applySmoothing(BigDecimal currentPrice, BigDecimal performanceScore) {
        ...
        BigDecimal newPrice = alpha.multiply(performanceScore)
    ...
    private BigDecimal parseFraction(Map<String, String> stats, String key, boolean numerator) {
        ...
        try {
            ...
            log.debug("Could not parse fraction part from stat '{}' value '{}' (numerator={})", key, value, numerator);
            return BigDecimal.ZERO;
        }
    }
